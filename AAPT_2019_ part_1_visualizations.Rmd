---
title: "AAPT Summer 2019 Stats WS"
...
To easily import data into R, click the "Import Dataset" button in the top-right panel in the "Environment" tab.  Import "ci_data_part1" file.
...
1.  What are the advantages of each type of plot?
2.  What are the disadvantages of each type of plot?
...
```{r}
tidy_ci_data <- gather(ci_data, key="time", value = "score", pre, post)
head(scores_data)
```

```{r}
ggplot(data=tidy_ci_data, aes(x=score, group=time, fill=time) ) + 
  geom_histogram(alpha=0.5, position = "identity") +
  theme(plot.title = element_text(hjust = 0.5))+
  labs(title = "Histogram of Pretest & Posttest Scores", y="Count", x= "Test Score (%)")
```

Activity! Use the ci_data and/or scores_data to create the following plots in ggplot:
(1) a scatter plot, 
(2) a box plot, and 
(3) a density plot.
...
```
Additional things to try if you finish early:
Change the colors
...
```{r}
tidy_ci_data$course <- sample(x=c("H","T"), prob = c(0.5,0.5))
